...
Vic.prototype.play = function(){
	...
	//this.vid.play();
	//画像ＵＲＬ
	//URLにクエリをつけることで、ブラウザのGifキャッシュをクリアする。
	var url='apng/sample05.gif?' + (new Date).getTime();
	//imgPreloaderオブジェクトを作る、名前は任意
	var imgPreloader=new Image();
	//imgPreloader.src=url;
	//imgPreloader.id = "img-" +this.vicId;
	// //onloadイベントハンドラ追加
	imgPreloader.onload=function() {
		//ロード完了で画像を表示
		$("#video-" + me.vicId).append(imgPreloader);
		me.pointerPlayForGif_();
	}
	//重要、最後に書く
	imgPreloader.src=url;
	//this.pointerPlay_();
...
Vic.prototype.pointerPlayForGif_ = function(){
	var me = this;
	var totalMilliSec = 0;
	//var vid = this.vid;
	console.log("1");
	...
	function doRender(){
		setTimeout(function(){
			if(totalMilliSec < 11 * 1000){
				if(!me.pauseFlag){
					for(var $itemObj of me.$itemObjs){
						if($itemObj.data("startTime") * 1000 < totalMilliSec && !$itemObj.data("added")){
							$itemObj.data("added",true);
							putItem($itemObj,me.videoObj.parent());
						}else if($itemObj.data("endTime") * 1000 < totalMilliSec && $itemObj.data("added")){
							$itemObj.stop();
					...
					totalMilliSec += 1000/fps;
				}
				doRender();
			}else{
				$("#video-" + me.vicId).empty();
			}
		}, 1000/fps);
	}
}
